...
var SignatureData = function () {
    ...
    this.GDPR_Group1 = ko.observable(false);
    this.GDPR_Group2 = ko.observable(false);
    this.GDPR_Group3 = ko.observable(false);
...
var DniData = function () {
    ...
    this.DNIAnversoBase64 = ko.observable().extend({
        required: {
            ...
            onlyIf: function () {
                return self.validateNow() && (((self.ScanDni() == 'false') && self.IdSendingWay() == DocumentSendingWayEnum.ADJUNTAR) || (self.ScanDni() == 'device')) && !self.DNIAnversoBase64();
            }
        }
    });

    this.DNIReversoBase64 = ko.observable().extend({
        required: {
            message: "Por favor, adjunta las dos fotografías de ambas partes del documento",
            onlyIf: function () {
                return self.validateNow() && (self.ScanDni() == 'device') && self.DNIAnversoBase64();
            }
        }
    });
    ...
    this.ViewModelToJSON = ko.pureComputed(function () {
        var data = {
            ...
            IdSendingWay: (self.ScanDni() == "true" || self.ScanDni() == "device") ? 0 : self.IdSendingWay(),
    ...
    this.getDeviceOS = ko.computed(function () {
        var deviceOS = Utils.GetMobileOS();
        return deviceOS;
    }, this);

    this.ShowDeviceDeliveryContent = ko.observable(false);
    this.ToggleDeviceDeliveryContent = function (value) {
        self.ShowDeviceDeliveryContent(value);
    }
    this.SwapDeviceDeliveryContinueButton = ko.computed(function () {
        var value = self.ScanDni() == 'device' && self.ShowDeviceDeliveryContent() == false;
        return value;
    }, this);
};
...
var BasicData = function (_assisted) {
    ...
    this.DNI = ko.observable().extend({
        ...
        CIF: {
            ...
            onlyIf: function () {
                return self.validateNow() && self.ApplicantType() == ApplicantTypeEnum.JURIDICA;
            }
        }
    });
    ...
    this.Email = ko.observable().extend({
        required: {
            ...
            onlyIf: function () {
                return self.validateNow() && !self.Assisted();
            }
        },
        email: {
    ...
    this.EmailCrii = ko.observable().extend({
        required: {
            ...
            onlyIf: function () {
                return self.validateNow() && self.Assisted() && (Utils.isNullOrEmpty(self.Email()) || Utils.isNullOrEmpty(self.MobilePhoneNumber()));
            }
        },
        email: {
            message: "Formato no válido para el email",
    ...
    this.ViewModelToJSON = ko.pureComputed(function () {
        var data = {
            ...
            DNI: self.DNI(),
            IDDocumentType: (self.ApplicantType() == ApplicantTypeEnum.FISICA) ? Utils.GetDocumentType(self.DNI(), 1) : 'CIF',
            Name: self.Name(),
...
var PersonalData = function (ProductInit) {
    var self = this;
    this.validateNow = ko.observable(false);
    ...
    this.IDDocumentExpirationDate = ko.observable(new CustomDate()).extend({
        RequiredCustomDate: {
            ...
            onlyIf: function () {
                return self.validateNow() && self.ShowDocumentExpirationDate() && self.ExpirationDateRequired();
            }
        },
    ...
    this.PhoneNumber = ko.observable().extend({
        pattern: {
            ...
            onlyIf: function () {
                return self.validateNow() && self.PhonePrefix().indexOf("+34") >= 0;
            }
        },
        NoRepeatNumber: {
            message: "Formato no válido para el teléfono móvil",
    ...
    this.ExpirationDateRequired = ko.observable(true);

    self.Birthday().Year.subscribe(function (newValue) {
        if (/*Utils.calculateAge(self.Birthday().ParseDate()) < 65*/ self.Birthday().getAge() < 65) {
...
var Address = function (addressType) {
    ...
    //    required: {
    //        message: "Por favor indique la extensión de la calle"
    //    },
    //})
...
var Applicant = function (ProductInit, assisted, deliveryOptionsHidden = false) {

    var self = this;

    this.validateNow = ko.observable(false);
    this.DeliveryOptionsHidden = ko.observable(deliveryOptionsHidden);
    this.Product = ko.observable(ProductInit);
    ...
    this.ApplicantPersonalData = ko.observable(new PersonalData(ProductInit));
    ...
    this.GDPR1 = ko.observable(false);
    this.GDPR2 = ko.observable(false);
    this.GDPR3 = ko.observable(false);
...
var ProductData = function (ProductTypes) {
    ...
    this.CheckEPSVProductTypeList = ko.observable("")
    this.FilterEPSVProductTypeList = ko.computed(function () {
        if (self.CheckEPSVProductTypeList() != undefined || self.CheckEPSVProductTypeList() != '') {
            var searchCP = self.CheckEPSVProductTypeList();
            var matchEPSVPostalCode = Object.values(EPSVPostalCodesPrefixEnum).find(pc => pc.toString() === searchCP.substring(0, 2));

            if (matchEPSVPostalCode != undefined) {
                return true;
            } else {
                return false;
            }
...
var RefundAccountNumberData = function () {
    ...
    this.ViewModelToJSON = ko.pureComputed(function () {
        var accountdata = {
            ...
            AccountNumberAcceptanceDate: self.AccountNumberAcceptanceDate()
        };
...
var ConvenienceTest = function (family, complexityDegree, isComplex, isAltaCrii) {
    ...
    if (self.UnansweredQuestions() == false) {
        self.showConfirmQuestion = ko.computed(function () {
            return self.ConvenienceResult() != undefined && self.ConvenienceResult() == false && self.IsComplex();
        }, this);
    }

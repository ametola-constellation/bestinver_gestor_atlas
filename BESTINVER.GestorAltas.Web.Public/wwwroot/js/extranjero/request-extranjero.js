...
Request.CreateBasicData = function (viewModel) {
    ...
    $.ajax({
        ...
        try {
            switch (data.status) {
                case 409:
                    viewModel.ErrorMessage('El DNI indicado ya tiene una solicitud de alta en Bestinver, para finalizar el proceso contacta con nosotros a través del teléfono 900 878 280 o del electrónico bestinver@bestinver.es, muchas gracias.');
                    Navigation.NextStep(viewModel.CurrentStep, 'Error');
                    break;
                case 403:
...
Request.SaveDniData = function (DniData, callback) {
    if (timeoutcheck) {
        customError(DniData.RequestId() + ": Timeout", apiConfig.API_Extranjero_SaveDNIData);
        location.href = timeouturl;
        return;
    }
    ...
    $.ajax({
        url: apiConfig.API_Extranjero_SaveDNIData,
        context: this,
        type: "POST",
        crossDomain: false,
        data: JSON.stringify(data),

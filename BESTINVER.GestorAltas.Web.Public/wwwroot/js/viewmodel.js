...
var ViewModel = function (ApplicantInit, ProductInit) {
    ...
    this.IsFCR = ko.observable(false);
    ...
    Self.CurrentEditApplicant().ApplicantContactData().FiscalAddress().PostalCode.subscribe(function (newvalue) {
        if (newvalue && newvalue != '') {
            Self.ProductModel().CheckEPSVProductTypeList(newvalue);
    ...
    this.AddExistsTutor = function () {
        ...
        if (found != undefined && !Self.listaOtherApplicants().some(function (elem) { return (elem.ApplicantBasicData().DNI() == found.ApplicantBasicData().DNI() && elem.ApplicantBasicData().RequestApplicantType() == RequestApplicantTypeEnum.TUTOR) })) {
            var app = Utils.CloneApplicant(found);
    ...
    this.UpdateCoownerAsTutor = function (a) {
        ...
        if (found != undefined) {
            if (found.IsOlderThan18()) {
                var app = Utils.CloneApplicant(a);
                ...
                if (found2 != undefined) {
                    ...
                    //Self.ExistsTutors().splice(index2, 1);
    ...
    this.UpdateApplicant = function () {
        ...
        if (!Self.ValidateAddressCotitular()) {
            ...
            Self.CurrentEditApplicant().ApplicantPersonalData().ExpirationDateRequired(Utils.GetDocumentType(Self.CurrentEditApplicant().ApplicantBasicData().DNI(), 1) == 'DNI');
                ...
                if (Self.CheckDocumentApplicants() && Self.CheckPhoneApplicants()) {
                    ...
                    switch (Self.CurrentType()) {
                        ...
                        case 23:
                            ...
                            break;
                        case 25:
                            GMT.PushEvent(PageEnum.COHEREDEROS, 'anadir', 'nudoPropietario');
                            if (Self.CurrentEditApplicant().IsSameAddress())
    ...
    this.CurrentApplicantSignatureTitle = ko.computed(function () {
        return 'Documentos personales del ' + Self.CurrentEditApplicant().ApplicantType() + ' ' + Self.CurrentEditApplicant().ApplicantBasicData().FullName();
    }, this);

    this.CurrentApplicantSendingDocumentTitle = ko.computed(function () {
        return 'Adjuntar documentos del ' + Self.CurrentEditApplicant().ApplicantType() + ' ' + Self.CurrentEditApplicant().ApplicantBasicData().FullName() + ' en formato JPG o PDF';
    }, this);

    this.CurrentApplicantSendingDocumentDeviceUploadTitle = ko.computed(function () {
        return 'Adjuntar documentos del ' + Self.CurrentEditApplicant().ApplicantType() + ' ' + Self.CurrentEditApplicant().ApplicantBasicData().FullName() + ' mediante fotografía';
    }, this);
    ...
    this.AllowAddOperation = ko.computed(function () {
        return Self.CurrentProduct().ProductOperationList().length < 16;
    }, this);
    ...
    this.SetSteps = function () {
        if (Self.MainApplicant().ApplicantBasicData().ApplicantType() == ApplicantTypeEnum.FISICA) {
            Self.StepsOrder = [
                "BasicData",
                "PersonalData",
                "ContactData",
    ...
    this.SetStepsOrder = function () {
        ...
        if (Self.MainApplicant().ApplicantBasicData().ApplicantType() == ApplicantTypeEnum.FISICA && Self.AltaCrii() == false) {
            this.RemoveStep('OperationIbanAssistedData');
        } else if (Self.MainApplicant().ApplicantBasicData().ApplicantType() == ApplicantTypeEnum.FISICA && Self.AltaCrii() == true) {
            this.RemoveStep('OperationIbanData');
        }
    };
    ...
    this.Init = function (crii, usercrii, mailcrii, ProductTypeList) {
        ...
        Self.IsFCR(((Self.CurrentProduct().Id() == 25) && Self.CurrentProduct().Family() == 12 && crii.toString() == 'false'));
    ...
    this.CheckApplicantQuestions = function (callback) {
        ...
        p.then(function (result) {
            ...
            //Console Self.MainApplicant().HasTest(1)
            console.log("Self.MainApplicant().HasTest(1): ", Self.MainApplicant().ApplicantHasTest(1));

            //Console Self.MainApplicant().Test().length
            console.log("Self.MainApplicant().Test().length: ", Self.MainApplicant().Test().length);
            ...
            //Console mainApplicantTests
            console.log("mainApplicantTests: ", mainApplicantTests);

            //Console knowledgeTestSteps
            console.log("knowledgeTestSteps: ", knowledgeTestSteps);
                ...
                if (!Self.AvoidTutorQuestions(a)) {
                    MasterData.getQuestionsSync(1, a.ApplicantBasicData().RequestApplicantType(), fondo, a.IsOlderThan18(), function (result) {
                        ...
                        Request.CheckConvenienceExistingApplicant(a.ApplicantBasicData().DNI(), productToCheck, function (data) {
                            ...
                            //Console a.DNI
                            console.log("a.DNI: ", a.DNI);

                            //Console result.length
                            console.log("getQuestionsSync otherApplicant result.length: ", result.length);
                            ...
                            //Console a.HasTest(1)
                            console.log("a.HasTest(1): ", a.ApplicantHasTest(1));

                            //Console a.Test.length
                            console.log("a.Test.length: ", a.Test.length);
                            ...
                            console.log("applicantTests.length: ", applicantTests.length);
        ...
        switch (currentstep) {
            ...
            case "AuthorizedData":
                ...
                coowners.forEach(function (a) {
                    ...
                    array_datalayer_applicant["provincia"] = array_datalayer_applicant["provincia"]
                        + a.ApplicantContactData().FiscalAddress().Province().toLowerCase()
                        + ';';
                ...
                array_datalayer_applicant["provincia"] = array_datalayer_applicant["provincia"].slice(0, -1);
            ...
            case "TutorData":
                switch (previuousstep) {
                    ...
                    case "CoInheritorDataData":
                        array_datalayer_applicant["tipoUsuario"] = "coheredero";
                        break;
                    case "UsufructuaryData":
    ...
    this.CheckEPSVOperation = function () {

    }


    this.AssignProductTypesByRole = function () {
        ...
        if (cotitulares) {
            ...
            if (!Self.ProductModel().FilterEPSVProductTypeList() && Self.MainApplicant().ApplicantBasicData().RequestApplicantType() != RequestApplicantTypeEnum.MINUSVALIDO) {
                productsTypesFilter = Self.ProductModel().ProductTypeList().filter(function (elem) {
                    return elem.Id() == ProductTypeEnum.FONDOS
                        || elem.Id() == ProductTypeEnum.INVERSIONLIBRE
                        || elem.Id() == ProductTypeEnum.PLANES
                });

                Self.ProductModel().ProductTypeList(productsTypesFilter);

            } else if (!Self.ProductModel().FilterEPSVProductTypeList() && Self.MainApplicant().ApplicantBasicData().RequestApplicantType() == RequestApplicantTypeEnum.MINUSVALIDO) {
                productsTypesFilter = Self.ProductModel().ProductTypeList().filter(function (elem) {
                    return elem.Id() == ProductTypeEnum.PLANES
                });

                Self.ProductModel().ProductTypeList(productsTypesFilter);

            } else if (Self.ProductModel().FilterEPSVProductTypeList() && Self.MainApplicant().ApplicantBasicData().RequestApplicantType() == RequestApplicantTypeEnum.MINUSVALIDO) {
                var ProductTypeMasterData = MasterData.ProductTypeList();

                var EPSVFilter = ProductTypeMasterData.filter(function (elem) {
                    return elem.Id() == ProductTypeEnum.EPSV || elem.Id() == ProductTypeEnum.PLANES
                });

                Self.ProductModel().ProductTypeList(EPSVFilter);
    ...
    this.goToNextStep = function (callback, notValidCallback) {
        ...
        if (currentStepOrder < (this.StepsOrder.length - 1)) {
            ...
            if (this.CurrentStep == 'OperationData') {
                var _this = this;
            ...
            } else if (Self.Product().ProductTypeId() == ProductTypeEnum.ASESORAMIENTOS && this.CurrentStep == 'OperationIbanAssistedData') {
                var _this = this;
    ...
    this.CheckConvenience = function (step) {
        console.log("CheckConvenience starts");
    ...
    this.ShowApplicantTest = function (testType, dni) {
        ...
        if (Self.MainApplicant().ApplicantBasicData().DNI().toUpperCase() + Self.MainApplicant().ApplicantBasicData().RequestApplicantType() == dni.toUpperCase()) {
            //Self.GetTest(Self.MainApplicant());
        ...
        if (Self.CurrentEditApplicant().ApplicantBasicData().RequestApplicantType() == RequestApplicantTypeEnum.TITULAR) {
            for (var i = 0; i < Self.listaOtherApplicants().length; i++) {
                if ((Self.listaOtherApplicants()[i].IsOlderThan18() || include_minors)
                    ...
                    && (Self.listaOtherApplicants()[i].ApplicantBasicData().RequestApplicantType() != RequestApplicantTypeEnum.USUFRUCTUARIO)) {
    ...
    this.CreateBasicData = function () {
        ...
        Self.MainApplicant().ApplicantPersonalData().ExpirationDateRequired(Utils.GetDocumentType(Self.MainApplicant().ApplicantBasicData().DNI(), 1) == 'DNI');
        $('#basicdata-dni').attr('readonly', true);
    };

    this.CreateDniData = function () {
        if (Self.DniData().ScanDni() == 'true') {
            Self.SaveDniData();
        } else {
            if (Self.DniData().IdSendingWay() == DocumentSendingWayEnum.ADJUNTAR || Self.DniData().ScanDni() == 'device') {

                var inputs;
                if ($('#documentationdelivery-id-custom').is(":visible")) {
                    inputs = $('#documentationdelivery-id-custom input:file');
                } else {
                    inputs = $('#documentationdelivery-id input:file');
                }
    ...
    this.SaveDniData_Complete = function (result) {
        if (result) {
            ...
            if (applicant != null) {
                customDebug(applicant, 'SaveDniData_Complete - next applicant');
                ...
                var isBstapp = mobileData.AppVersion && (mobileData.AppVersion.indexOf("bstapp") != -1 || mobileData.AppVersion.indexOf("bestinver") != -1);

                if (isBstapp) {
                    $('#documentationdelivery-others-content').addClass('hidden');
                    $('#documentationdelivery-others-content').css('display', '')
                    $('#documentationdelivery-device-content').addClass('hidden');
                    $('#documentationdelivery-device-content').css('display', '');

                    Self.DniData().ShowDeviceDeliveryContent(false);
                }

                if (applicant.ApplicantBasicData().ApplicantType() == ApplicantTypeEnum.FISICA) {
                    $('#documentationdelivery-others-content').removeClass('hidden');
                }
            }
            else {
                customDebug(vm.RequestId(), 'SaveDniData_Complete - Completed process');
                Self.CompleteProcess();
                Self.goToNextStep();
            }
        }
        else {
            customDebug(vm.RequestId(), 'SaveDniData_Complete - Error');
            var txt = '';
            if (Self.DniData().ScanDni() == 'true') {
                txt = '';
            }
    ...
    this.CreateSignatureData = function () {
        // TODO Test SetGDPRAllApplicants
        if (Self.SignatureData().IdDocumentSignatureType() == DocumentSignatureTypeEnum.MANUSCRITA) {
            if (Self.SignatureData().GDPR_Group1() && Self.SignatureData().GDPR_Group2() && Self.SignatureData().GDPR_Group3()) {
                Self.SendGDPRAnalitics(Self.SignatureData().GDPR_Group1(), Self.SignatureData().GDPR_Group2(), Self.SignatureData().GDPR_Group3());
                Self.SetGDPRAllApplicants(Self.SignatureData().GDPR_Group1(), Self.SignatureData().GDPR_Group2(), Self.SignatureData().GDPR_Group3());
            } else {
    ...
    this.SetGDPRAllApplicants = function (gdpr1, gdpr2, gdpr3) {
        Self.SetGDPRApplicants(Self.MainApplicant().ApplicantBasicData().DNI(), gdpr1, gdpr2, gdpr3);
        Self.listaOtherApplicants().forEach(function (ap) {
            Self.SetGDPRApplicants(ap.ApplicantBasicData().DNI(), gdpr1, gdpr2, gdpr3);
        });
    };
    ...
    this.SignatureApplicant = function () {
        ...
        if (applicant != null) {
            customDebug(applicant, 'Sign process continue - set next applicant');
            ...
            customDebug(Self.MainApplicant(), 'Sign process completed - Go to uploads Documents');
            Self.DniData().validateNow(false);
            Self.CurrentEditApplicant(Self.MainApplicant());
            if (!Self.MainApplicant().IsOlderThan18() || isIE || Self.MainApplicant().DeliveryOptionsHidden()) {
    ...
    this.InitDocumentApplicant = function (applicant) {
        ...
        if (!applicant.IsOlderThan18() || isIE || applicant.DeliveryOptionsHidden()) {
            Self.DniData().ScanDni("false");
        }
    };
    ...
    this.ManageTransaction = function (id) {
        Self.DniData().ElectronicIDRequestId(id);
        $("#applicant-document-view").hide();
        Self.DniData().ScanDni("true");
    };

    this.SendGDPRAnalitics = function (gdpr1, gdpr2, gdpr3, isModal) {
        var eventCategGdpr =
            'gdpr 1 alta ' +
            ((gdpr1 === true) ? 'ok' : 'ko') +
            ':gdpr 2 alta ' +
            ...
            ':gdpr 2 alta ' +
            ((gdpr3 === true) ? 'ok' : 'ko');
        GMT.PushEvent((isModal ? 'modal' : PageEnum.FIRMA_DOCUMENTOS_PIN), 'gdpr alta', eventCategGdpr);
    };
...
var SelectProductViewModel = function (ProductTypes, disclaimer) {
    ...
    this.GoToPersonalDataView = function () {
        if (self.ProductModel().Product() == null) {
            ...
            var url = routeConfig.GA_Register_PhysicalPersons_PersonalData + "/" + btoa("ProductId=" + self.ProductModel().Product());
            location.href = url;
        }
    };
};
...
var RecoverySignatureViewModel = function () {
    ...
    this.DeliveryOptionsHidden = ko.observable(false);
    this.IsRubricaeEventsAttached = ko.observable(false);
    this.Signed = ko.observable(false);

    this.Init = function (RequestId, docs, productType, minors, Birthday, hideDeliveryOptions) {
        this.DeliveryOptionsHidden(hideDeliveryOptions);
        Self.PdfDocumentData(docs);
        Self.InitSignApplicantDocument();
        Self.DniDataView().Dni(Self.PdfDocumentData().Dni());
        Self.DniDataView().RequestId(RequestId);
        Self.DniDataView().MoveStatus(false);//Se ha quitado el if que comprobaba esta variable en RegisterService SetApplicantDocumentSent
        ...
        var date = moment(Birthday, 'YYYY-MM-DD', true); //new Date(Birthday);
        ...
        if (isIE || this.DeliveryOptionsHidden()) {
    ...
    this.hideRubricaeSubtitle = function () {
        var subtitleRubricaeElement = document.querySelector('.ro-subtitle-rubricae');
        if (subtitleRubricaeElement) {
            subtitleRubricaeElement.style.display = 'none';
        }
    }
    ...
    this.CreateDniData = function () {
        if (Self.DniDataView().ScanDni() == 'true') {
            Self.SaveDniData();
        }
        else {
            if (Self.DniDataView().IdSendingWay() == DocumentSendingWayEnum.ADJUNTAR || Self.DniDataView().ScanDni() == 'device') {
    ...
    this.CurrentApplicantSendingDocumentDeviceUploadTitle = ko.computed(function () {
        return 'Adjuntar documentos del ' + Self.CurrentEditApplicant().ApplicantType() + ' ' + Self.CurrentEditApplicantFullName() + ' mediante fotografía';
    }, this);

    this.ManageTransaction = function (id) {
        Self.DniDataView().ElectronicIDRequestId(id);
        $("#applicant-document-view").hide();
    };
...
var SendDocumentViewModel = function () {
    ...
    this.Init = function (RequestId, AplicantDni, ApplicantName, IsMinor, Birthday, minors, RequestApplicantType, hideDeliveryOptions) {
        this.DeliveryOptionsHidden(hideDeliveryOptions);
        ...
        var isIE = /*@cc_on!@*/false || !!document.documentMode;
        if (IsMinor == "true" || isIE || this.DeliveryOptionsHidden()) {
            Self.DniDataView().ScanDni("false");
            $('#documentationdelivery-others-content').removeClass('hidden');
        }
        else {

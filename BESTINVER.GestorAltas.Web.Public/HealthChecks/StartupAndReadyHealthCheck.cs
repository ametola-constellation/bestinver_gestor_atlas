...
namespace BESTINVER.GestorAltas.Web.Public.HealthChecks
{
    /// <summary>
    /// 
    /// </summary>
    public class StartupAndReadyHealthCheck : IHealthCheck
    {
        ...
        private async Task Warmup(CancellationToken cancellationToken)
        {
            ...
            {
                var services = GetServices(_services);
                foreach (var singleton in services.Where(x => x.Name.Equals("IRuleProcessor")))
                {
                    try
                    {
